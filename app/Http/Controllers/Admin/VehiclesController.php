...
class VehiclesController extends Controller {
	...
	public function index() {
	    ...
	    if(Auth::user()->id==1){
	      $data=VehicleModel::orderBy('id','desc')->get();  
	         ...
	         $data=VehicleModel::orderBy('id','desc')->where('user_id',Auth::user()->id)->get();
	    }
	...
	public function alignment(){
	    ...
	    if(Auth::user()->id==1){
	      $data=Alignment::orderBy('id','desc')->get();  
	    }else{
	        $data=Alignment::orderBy('id','desc')->where('user_id',Auth::user()->id)->get();
	    }
	   
	      
	     
		return view("vehicles.alignment",compact('data'));
	    ...
	    if(Auth::user()->id==1){
	      $data=Fleet::orderBy('id','desc')->get();  
	    }else{
	        $data=Fleet::orderBy('id','desc')->where('user_id',Auth::user()->id)->get();
	    }
	   
	      
	     
		return view("vehicles.fleet",compact('data'));
	    
	}
	
	public function edit_fleet(Request $request){
	 $data=Fleet::orderBy('id','desc')->where('id',$request->id)->first();
	 

	 	return view("vehicles.edit_fleet",compact('data'));
	    
	}
	
			public function edit_fleet_update(Request $request){
	      
	    
	  
		$vehicle= Fleet::find($request->id);
	
		$vehicle->fleetname=$request->fleetname;
	
		$vehicle->start=$request->start ?? '';
		$vehicle->asset=implode(',',$request->service ?? '');
	  $vehicle->models=$request->models ?? '';
	   $vehicle->end=$request->end ?? '';
	...
	return Redirect::route("aglign.ment2");
	}
	
	
		public function fleet_store(){
		...
		return view("vehicles.fleet_create");
	...
	public function alignment_store(){
		...
		return view("vehicles.alignment_create");
	    
	}
	
	
	public function store_alignment(Request $request){
...
$currentTime = date( 'h:i A', time () );
	   ...
	   if($request->hasfile('upload'))
		...
		$vehicle= new Alignment();
	
		$vehicle->vehicle_no=$request->searchno;
	
		$vehicle->odometer=$request->odometer ?? '';
		$vehicle->date=$request->date ?? '';
		$vehicle->time=$currentTime ?? '';
		
	   
	    $vehicle->user_id=$user_id ?? '';
	    	$vehicle->image=$filename;
	    		$vehicle->month=$today->month ?? '';
	  
	   $vehicle->save();
        }
	    
	return Redirect::route("aglign.ment");
	...
	public function store(Request $request) {
	    ...
	    $check=VehicleModel::where('vehicle_no',$request->no)->first();
	    if(!empty($check)){
	        	return back()->with('success','Your Entry Already exists');
		...
		$vehicle->vehicle_no=$request->no;
		$vehicle->manufacturar=$request->manufacturar ?? '';
		$vehicle->odometer=$request->odometer ?? '';
		$vehicle->date=$request->date ?? '';
	   $vehicle->wheeler_no=$request->wheeler ?? '';
	    $vehicle->user_id=$user_id ?? '';
    ...
    VehicleModel::find($id)->delete();
		...
		public function alignmentdelete(Request $request){
	    $id=$request->id;
    Alignment::find($id)->delete();
         return back();
	}
