...
class ExpenseController extends Controller {
	...
	public function index(Request $request) {
	     ...
	     if(Auth::user()->id==1){
	       ...
	       $data['inventory']=Expense::orderBy('id','desc')->get();
	    }else{
	        
	        
	         $data['inventory']=Expense::orderBy('id','desc')->where('user_id',Auth::user()->id)->get();
	    }
	...
	$inventory= new Expense();
	$inventory->vehicle_id=0;
	$inventory->amount=0;
	$inventory->user_id=Auth::id();
	$inventory->date=$request->date ?? '';
	$inventory->comment='';
	$inventory->expense_type=1;
	$inventory->type=0;
	$inventory->qty=$request->revenue ?? 0;
	$inventory->patch=$request->expense_type  ?? '';
    $inventory->save();
