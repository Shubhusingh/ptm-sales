...
class ReportsController extends Controller {
	...
	public function monthly_alignment() {
		...
		return view("reports.alignmentreport", $data);
	}
...
if(Auth::user()->id ==  1){
 		    $data['search']=Alignment::orderBy('id','desc')->where('month',$request->month)->get();
   
}else{
    		    $data['search']=Alignment::orderBy('id','desc')->where('month',$request->month)->where('user_id',Auth::user()->id)->get();
...
*/
	...
	public function monthly_post(Request $request) {
		...
		if(Auth::user()->id ==  1){
	    $data['search']=VehicleGroupModel::orderBy('id','desc')->where('month',$request->month)->get();
   
}else{
	    $data['search']=VehicleGroupModel::orderBy('id','desc')->where('month',$request->month)->where('user_id',Auth::user()->id)->get();
...
$data['month']=$request->month ?? '';
	...
	public function fuel() {
		...
		$data['month_select'] = date("n");
		$data['years'] = $y;
		return view('reports.fuel', $data,compact('search'));
	}
