      ...
      <div class="card-body">
        ...
        <div class="row" style="height:500px; overflow-y:auto;">  
          <table class="table table-bordered">
            ...
            <tbody>
              @php $new_names = ['Employee details','vehicles','VehicleGroup','vehicle_type','Fleet Operations','reports','fitter']; @endphp
              @php $new_modules = array_combine($modules,$new_names); @endphp
              
              {{-- @dd($new_modules) --}}
              @foreach($new_modules as $row=>$val)
              <tr>
                <th scope="row" class="table-row">{!! Form::label('permission',__('Modules.'.$val), ['class' => 'col-xs-5 control-label']) !!}</th>
                <td>
